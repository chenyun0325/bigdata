...
rows = metadb_hook.get_pandas_df("""
    ...
    from metadata.job t1
           join metadata.datasource t2 on t1.source_id = t2.id and t2.is_del = false
           left join metadata.ods_table t4 on t1.source_table_id = t4.id and t4.is_del = false
           join metadata.schedule t5 on t1.schedule_id = t5.id and t5.is_del = false
        ...
        if row.job_type == 'SPARK':
            ...
            job = BashOperator(
                ...
                bash_command="""ssh {} << EOF
                                 source ~/.bash_profile; /usr/spark-2.4.3/bin/spark-submit --master {} \
                                --executor-memory {} \
                                --driver-memory {} \
